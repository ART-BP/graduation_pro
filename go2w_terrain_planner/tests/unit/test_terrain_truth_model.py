...
def test_task_goal_places_feature_between_start_and_goal() -> None:
    generator = TerrainTruthModel(
        8,
        "cpu",
        TerrainTruthConfig(size=24, enabled_terrain_names=("step",)),
    )
    ids = torch.arange(8)
    generator.reset(ids)
...
def test_reset_respects_explicit_terrain_probabilities() -> None:
    generator = TerrainTruthModel(256, "cpu", TerrainTruthConfig(size=12))
...
def test_reset_matches_requested_terrain_mixture() -> None:
    torch.manual_seed(7)
    count = 4000
    generator = TerrainTruthModel(count, "cpu", TerrainTruthConfig(size=12))
...
def test_reset_rejects_probability_on_disabled_terrain() -> None:
    ...
    generator = TerrainTruthModel(
        count,
        "cpu",
        TerrainTruthConfig(size=12, enabled_terrain_names=("flat",)),
    )
    env_ids = torch.arange(count)
...
def test_pit_geometry_progresses_from_easy_to_full_range() -> None:
    ...
    generator = TerrainTruthModel(count, "cpu", cfg)
    env_ids = torch.arange(count)
    generator.reset(
        env_ids,
...
def test_wall_width_leaves_bounded_detour_corridor() -> None:
    ...
    generator = TerrainTruthModel(1, "cpu", cfg)
...
def test_mixed_terrain_contains_slope_step_and_obstacle() -> None:
    generator = TerrainTruthModel(1, "cpu", TerrainTruthConfig(size=12))
...
def test_stairs_support_upward_and_downward_traversal() -> None:
    generator = TerrainTruthModel(
        2,
        "cpu",
        TerrainTruthConfig(size=12, enabled_terrain_names=("stairs",)),
    )
    ...
    assert ground[1, 0].item() == pytest.approx(4.0 * 0.45 * 0.2)
    assert ground[1, 1].item() == pytest.approx(1.0 * 0.45 * 0.2)


def test_domain_randomization_scale_gates_friction() -> None:
    generator = TerrainTruthModel(
        2,
        "cpu",
        TerrainTruthConfig(
...
def test_truth_query_is_independent_from_actor_observation() -> None:
    generator = TerrainTruthModel(
        1,
        "cpu",
        TerrainTruthConfig(size=24, enabled_terrain_names=("wall",)),
    )
    generator.terrain_type[:] = 6
    generator.amplitude[:] = 0.8
...
def test_height_range_uses_within_cell_surface_span() -> None:
    cfg = TerrainTruthConfig(size=20, extent_m=10.0, enabled_terrain_names=("ramp",))
    generator = TerrainTruthModel(1, "cpu", cfg)
...
def test_stairs_use_single_riser_for_collision_height() -> None:
    ...
    generator = TerrainTruthModel(
        count,
        "cpu",
        TerrainTruthConfig(size=20, enabled_terrain_names=("stairs",)),
    )
...
def test_ground_reference_uses_supporting_mesh_height() -> None:
    generator = TerrainTruthModel(
        1,
        "cpu",
        TerrainTruthConfig(size=24, enabled_terrain_names=("step",)),
