...
from go2w_terrain_planner.mapping.grid_preprocessor import preprocess_grid_map
...
def test_published_ground_is_already_ground_referenced() -> None:
    ground = np.array([[0.0, 0.2]], dtype=np.float32)
    height_range = np.zeros_like(ground)
    observed = np.ones_like(ground)
