...
def test_default_actor_and_critic_dimensions_are_explicit() -> None:
    ...
    assert DEFAULT_POLICY_OBSERVATION_DIMENSION == 280045
    assert CRITIC_OBSERVATION_DIMENSION == 64
    assert sum(CRITIC_OBSERVATION_COMPONENTS.values()) == 64
    assert critic_observation_dimension(
