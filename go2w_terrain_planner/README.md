...
- 用非物理Xform作为 Go2W 外形和传感器坐标占位对象；
- 用独立的高层速度执行模型模拟速度滞后、加速度限制、摩擦、有限地形阻力、进入角度、跟踪噪声和卡住；
- 用单级突变、障碍高度、坑深和足迹支撑变化分别构造单调的碰撞与失稳结果；
- 在 Isaac Lab 中预先生成平地、坡道、台阶、楼梯、粗糙地面、深坑、墙体、立柱和混合地形的真实三角网格库，课程每回合直接选择对应的物理地形块；
- 使用 Isaac Lab `RayCaster` 持有原生 Warp 三角网格和XT16射线模式，对该网格执行16线×每线2000个方位角，共32000条独立射线的首回波求交；射线位姿直接来自环境维护的权威规划状态，不再经由重复的 PhysX 刚体视图反读。场景传感器采用惰性更新，环境每个决策步只执行一次权威射线计算。随后按实机低分位地面与稳健高度跨度规则投影为四通道局部地图；瞬时射线帧用于近似实机经 FAST-LIO2 去畸变后的注册点云；
- Actor 路径已删除解析式稠密观测、粗角度锚点和插值补点，每个有效点均来自物理网格的实际射线命中；
...
- 最近8步（0.8 s）运动增量与速度指令组成`[dx,dy,dyaw,v_cmd,w_cmd]`序列，由128维GRU编码；GRU只处理低维运动历史，不处理高分辨率地图；
- 局部目标接口仍覆盖`0～10 m`，但episode采样范围由每个能力阶段显式定义：从阶段1的`1.0～2.5 m`扩展到阶段9的`5.0～10.0 m`，不再用地形枚举值线性推导目标距离；
- 所有几何原语都以17级静态物理网格细粒度逼近阶段内连续难度；每类地形包含8种前向位置、横向偏置、宽度或通行方向变体，局部障碍的目标路线经过真实特征区域；楼梯回合以50%概率生成几何一致的上楼或下楼方向，第9阶段才随等级内难度逐步打开摩擦、位姿、测距、丢束、丢回波和执行跟踪噪声；
- 深坑、墙体、立柱、混合障碍和多路径障碍的进展与航向奖励使用安全余量扩张后的矩形障碍及两侧绕行点构造无碰撞引导；最终成功仍仅由真实目标欧氏距离判定，Actor也不会接收该解析真值；64维特权 Critic 同时接收导航势能、地形几何、风险、未知比例、速度跟踪误差、指令历史和运动历史；
...
- PPO 以 `5e-5` 为初始学习率，并根据 `desired_kl=0.01` 自适应调节，配合`0.1`裁剪范围和`0.5`梯度范数限制约束更新幅度；
- PPO每轮每环境收集96步，32环境时形成3072条样本，并使用8个mini-batch和3次学习epoch，以增加包含障碍交互的单轮轨迹长度并控制扩大模型后的梯度方差；
...
- 基于 RTX 材质、反射强度、束散角、扫描时序和多回波的完整 XT16 光学仿真；当前已按逐线标定角表生成原生几何射线，但回波仍是理想三角网格首交点。

这两项属于后续保真度扩展。替换时保持四通道地图和 `[v_cmd,w_cmd]` 接口不变即可。资产边界定义在 `robots/go2w_cfg.py`，传感器边界是 `TerrainNavigationSceneCfg.lidar`与 `IsaacRaycastLidarMapper.project()`。
...
这里的 `ground_height` 已由真实地图节点或仿真 RayCaster 点云投影器转换为相对于机器人脚下参考地面的高度，平地约为 0；共享预处理器不会再减 FAST-LIO 的 IMU 高度。

仿真端的观测源固定为 Isaac Lab 原生 `RayCaster`。处理后点云保存在
`IsaacRaycastLidarMapper.last_pointcloud`，默认形状为 `[B,32000,3]`，坐标系为当前机器人局部坐标系，
无回波射线填充为 `NaN`；`last_hit_mask` 提供有效回波掩码。射线穿越距离独立形成观测掩码，仅实际命中点能够更新高度。地形执行、碰撞和失稳判定使用与当前物理网格元数据同步的特权真值，Actor 不接收该真值。
纯PyTorch投影器随后执行12 m输入裁剪、0.05 m栅格分组、10%
地面分位数和5%～95%高度跨度估计。每格至少需要两个真实回波才产生有效地面与高度差，避免单点栅格被误认为零高度差。它不链接ROS、PCL、Grid Map或`local_map_ws`，但保持主要观测语义一致。
...
cmd_history   [B, 8, 2]    # 历史指令除以相同物理速度尺度
motion_history[B, 8, 3]    # dx,dy除以5 m，dyaw除以pi
```

Actor辅助输入在拼接前使用固定物理尺度归一化，不依赖训练数据的运行均值，因此训练、评估和实机部署具有相同语义。默认 Actor 展平维度为 `280045`，PPO中只保存单张7通道紧凑地图。12帧融合历史只存在于环境表示模块内部，不随96步rollout重复存储。Critic 使用独立的64维特权状态，这些仿真真值不会进入 Actor 或 ONNX 模型。
其中目标距离以 `10 m` 归一化并截断到 `[0,1]`。机器人接近目标时能够自然观测到
接近零的距离；各课程阶段的最小目标距离只约束episode初始采样，不限制运行时接口。
...
```text
课程阶段 → 选择已预生成的真实三角网格地形块
物理网格 → Isaac Lab RayCaster的32000条独立首回波射线 → 局部点云
  ...
  → 位姿、碰撞、失稳、卡住和目标进展
  → PPO 奖励与终止
物理网格同步特权真值 → 仅供执行模型、Critic、奖励和终止使用
```
...
- `sensor.yaml`：每帧独立射线数、XT16逐线标定角、水平分辨率、量程、安装高度、点云裁剪、分位数投影，以及射线/回波丢失和位姿扰动；
...
当前训练run为 `phase13_raycast_reward_diversity`。0.8 s历史使策略观测与Critic输入维度发生变化，网络/checkpoint接口版本已升级为7；不能对版本6及以前的checkpoint使用`--resume`，应从头训练版本7。
...
```text
input:  policy_observation [B, 280045]
...
```text
/data/go2w_training/
├── runs/rsl_rl/go2w_terrain_navigation/<timestamp>_phase13_raycast_reward_diversity/
│   ├── configs/          # 六份配置快照
...
2. 用完整 Go2W articulation 和底层速度跟踪器替换当前非物理可视化代理与 `VelocityExecutionModel`。
